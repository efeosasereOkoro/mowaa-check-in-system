...
// Narrow-only: sticky "Add user" bar + a full-height sheet wrapping AddUserForm.
export default function AddUserSheet() {
  ...
  return (
    <>
      ...
      {open && (
        <div role="dialog" aria-label="Add user" style={{ position: 'fixed', inset: 0, zIndex: 1000, background: '#fff', display: 'flex', flexDirection: 'column' }}>
          <div style={{ flex: 'none', height: 56, borderBottom: '1px solid #E0E0E0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px' }}>
            <span style={{ fontSize: 16, fontWeight: 600 }}>Add user</span>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
            <AddUserForm sheet onSuccess={() => setOpen(false)} />
          </div>
        </div>
      )}
    </>
