...
export default function HealthRoster({ roster }: { roster: HealthRosterRow[] }) {
  ...
  const mobileRows: CollapsibleRow[] = rows.map((r) => ({
    key: r.id,
    primary: (
      <>
        {r.emergencyToday && <EmergencyDot />}
    ...
    status: <span style={{ fontSize: 13 }}>{statusLabel[r.status]}</span>,
    detail: (
      <>
        <DetailGrid
          items={[
            { label: 'Tag', value: <span style={{ fontFamily: 'monospace' }}>{r.tagCode ?? '—'}</span> },
            { label: 'Allergies / conditions', value: r.healthDetails || '—' },
  ...
  return (
    <div>
      ...
      />
      <CollapsibleTable desktop={desktop} rows={mobileRows} statusWidth={112} empty="No children match." />
    </div>
  );
}
